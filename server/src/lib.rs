...
use db::*;
use rocket_db_pools::sqlx::{Postgres, Pool, Row};
...
//                 response = format!("{}\n{}", response, line)
//             }
//             // println!("line: {:?}", line.to_string().split("\n").collect::<Vec<&str>>());
//         }
//         if line.is_empty() {
//             data_found = true
//         };
//     }
//     println!("response");
//     println!("{:?}", response.trim_matches(char::from(0)).to_string().split("\n").collect::<Vec<&str>>());    
...
pub async fn get_tank_build_status_by_url(
    pool: &Pool<Postgres>, 
    url: &str,
) -> TankBuildStatus {
