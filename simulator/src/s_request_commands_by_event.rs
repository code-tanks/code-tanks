...
pub fn request_commands_by_event(
    mut query: Query<(Entity, &mut CommandSource, &mut EventSink, &mut Client, &mut Health)>,
) {
    for (entity, mut command_receiver, mut event_sender, mut client_connection, mut health) in &mut query {
